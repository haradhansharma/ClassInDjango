class ReportPDFData:
    ...
    def first_page(self, c, doc):
        ...
        c.drawString(self.M, self.M, "Home Page / %s" % self.t_additional)
        c.restoreState()
    ...
    def later_page(self, c, doc):
        ...
        self.top_string(c, doc)
        c.setFont('Times-Roman',9)
        c.drawString(self.M, self.M, "Page %d %s" % (doc.page, self.t_additional))
        c.restoreState()
    ...
    def desclimar_and_content(self):
        ...
        data = [
            ...
            ListFlowable(
            [
            ListItem(Paragraph('<a href = #dis >Disclaimer</a>', self.stylesN),bulletColor='green',value='square'),
            ListItem(Paragraph('<a href = #lsb >Latest Status Of Biofuel</a>', self.stylesN),bulletColor='green',value='square'),            
            ListItem(Paragraph('<a href = #bs >Biofuel Summary</a>', self.stylesN),bulletColor='green',value='square'),            
            ListItem(Paragraph('<a href = #qsf >Question Specific Feedack</a>', self.stylesN),bulletColor='green',value='square'),
            ListItem(Paragraph('<a href = #doa >Details of Activities</a>', self.stylesN),bulletColor='green',value='square'),
            ListItem(Paragraph('<a href = #bh >Biofuel History</a>', self.stylesN),bulletColor='green',value='square'), 
            ],
            bulletType='bullet',
            start='square',           
            ),           
            
            Spacer(self.PH/1.85,self.PH/1.85),            
            Paragraph('<a name="dis"/>DISCLAIMER', self.SectionT),
            self.uline(),
            Paragraph(text, self.stylesN),
            Paragraph(text2, self.stylesN),            
            Spacer(0.15*inch,0.15*inch),
        ]
    ...
    def grape_status(self):
        ...
        drawing = Drawing(self.PW - self.M - self.M - self.M/2, 280)
        ...
        bc.bars.symbol.fillColorStart = red
        ...
        bc.bars.symbol.strokeColor = red        
        ...
        bc.bars[2].symbol.fillColorStart = green
        ...
        bc.bars[2].symbol.strokeColor = green
    ...
    def todos(self):       
        ...
        for na in na_ac:
            ...
            for quotation in na.get_quotations():
                quotations += f'<u><a href="{quotation.get_absolute_url()}">Quot#{quotation.id}::{quotation.price}</a></u> , '             
            data.append(Paragraph(todo_title, self.stylesN))
            data.append(self.uline100())
            data.append(Paragraph(quotations, self.LeftIndent))            
    ...
    def summary_statement(self):
        ...
        for el in eva_label:            
            title = Paragraph(f'<a name="{el.id}l"/><font color="darkgreen">{el.label.label.upper()}</font>', self.stylesH2)
            data.append(title)
            data.append(self.ulineDG100())            
            for es in el.evalebelstatement_set.all():
                if es.statement is not None:
                    st = Paragraph(es.statement, self.stylesN)
                    data.append(st)                                    
                if es.next_step is not None:                    
                    ns = Paragraph(es.next_step, self.stylesN)
                    data.append(ns)
                    data.append(Spacer(0.15*inch,0.15*inch))         
    ...
    def question_specific_feedback(self):
        ...
        for e in evaluation:
            ...
            for quotation in e.question.get_quotations:
                quotations += f'<u><a href="{quotation.get_absolute_url()}">Quotation#{quotation.id}::{quotation.price}</a></u> , '                  
            data.append(Paragraph(quotations, self.LeftIndent))
            quotationsr = ''
            for quotation in e.question.get_related_quotations:
                quotationsr += f'<u><a href="{quotation.get_absolute_url()}">Quotation#{quotation.id}::{quotation.price}</a></u> , '       
            data.append(Paragraph(quotationsr, self.LeftIndent))            
            comments = ''
            for qa in e.get_question_comment:
                comments += f'<p>{qa.comments}</p>'
            data.append(Paragraph(f'<b>Self Comment :</b>{comments}', self.stylesN))
            data.append(Paragraph(f'<b>GFVP Feedback :</b> {e.option.statement}', self.stylesN))
    ...
    def details_of_activities(self):
        ...
        for e in next_activities:           
            ...
            for rq in e.related_questions.all():    
                rqs.append(ListItem(Paragraph(f'{rq.name}', self.LeftIndent),bulletColor='red',value='square'))    
            ...
            data.append(q_data)
            data.append(Paragraph(f'<b>Descriptions :</b> {e.descriptions}', self.stylesN))
